...
模型提供方发布时不会缺少模型来源、模板、标签、币种或审核入口。
...
15. 按模型能力开启高级能力。
    - 仅在模型已验证支持时开启思考模式。
    - Function Calling（函数调用、工具支持）仍在规划中，当前版本不作为可用能力配置。
16. 设置 Token 限制。
    - 按模型官方能力填写最大上下文、最大输入和最大输出。
    - 避免填写超过模型能力的数值，导致 模型提供方 发布模型后测试失败。
17. 选择官方原生协议。
    - 可多选。
...
5. 添加地域信息。
    ...
    - 模型提供方 发布模型时需要选择实际调用地域。
6. 填写地域标识和地域名称。
7. 填写 **BASE URL**。
    - 填写模型服务的基础 API 地址。
    - 不要填写具体模型或单个接口路径。
8. 填写 API 密钥地址。
    - 填写获取 API Key 的官方页面地址，用于引导 模型提供方 获取密钥。
9. 填写 API 文档地址。
    - 填写模型服务文档地址，用于后续核对协议、接口地址 和参数。
...
这里不要填写或公开真实 API Key。正式文档中只保留占位符，真实密钥由 模型提供方 在发布模型或测试协议时按权限填写。
...
模板用于把模型作者、模型来源、地域、元模型、模型源 ID 和协议能力串起来。模型提供方 发布模型时，如果这里缺少模板或模板配置不完整，发布流程中可能无法选择目标配置，或协议测试失败。
...
1. 发布收费公有模型前，先与平台设置负责人确认当前平台统一使用的计价币种。
2. 核对收费模型使用的币种是否与运营定价口径一致。
    - 平台币种会影响交易和费用展示。
3. 如需调整币种，按当前部署环境提供的平台级设置入口处理。
4. 点击最终 `保存`、`提交` 或 `确定` 前，再次确认对运营定价、用户展示和历史记录展示的影响。
5. 文档或学习场景只记录确认结果，不记录真实客户价格、结算策略或内部财务配置。

计价币种会影响全平台交易和费用展示。已有线上交易后，不建议随意调整。

## 6. 审核模型发布申请

1. 模型提供方 完成模型发布并提交审核后，在左侧菜单中进入 **"审批 > 模型审核"**。
2. 在模型审核列表中找到待审核模型。
    - 可按待审核、审核通过、审核失败筛选。
    - 优先处理待审核模型。
3. 如需缩小范围，按模型类型筛选。
...
6. **审核入口可处理**：平台运营方 可以在 **"审批 > 模型审核"** 中查看并处理模型发布申请。

## 8. 牢记这几点

1. 不要把 API Key、AK/SK、Cookie、真实请求头值或账号密码写入公开文档。
2. 模型来源、元模型和模板必须能对应上，否则 模型提供方 发布时会缺少可选项或无法通过协议测试。
3. 公有模型发布后会对外可见，审核前必须核对名称、标签、计费、限流和协议测试结果。
4. 本文只适用于 模型及 AI 服务 的模型发布预配置。
...
| 检查项 | 通过标准 |
| --- | --- |
| 1 | 元模型、来源、模板、标签和币种均启用且相互一致。 |
| 2 | 模型提供方测试账号能看到预期发布选项。 |
| 3 | 平台运营方能定位并处理测试审核申请。 |
...
| 现象 | 优先检查 |
| --- | --- |
| 提供方缺少来源或模板 | 状态、地域、模型类型、来源模板关联和账号范围 |
| 协议默认值错误 | 厂商文档、接口地址、模型 ID、模态、限制和参数 |
