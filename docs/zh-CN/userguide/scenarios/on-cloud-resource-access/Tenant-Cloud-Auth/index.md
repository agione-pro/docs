...
3. 在「选择云平台」下拉列表中，勾选需要授权的当前已支持云平台（支持多选，如阿里云、AWS、{{DOCS_CLOUD_PLATFORM_ZH}}）；华为云接入当前暂不支持。
4. 选择授权范围（二选一）：
...
| 字段名称 | 字段类型 | 示例 | 说明 |
|----------|----------|------|------|
| 选择云平台 | 多选下拉 | `阿里云`、`AWS`、`{{DOCS_CLOUD_PLATFORM_ZH}}` | 必填，支持同时选择多个当前已支持的云平台；华为云接入当前暂不支持 |
