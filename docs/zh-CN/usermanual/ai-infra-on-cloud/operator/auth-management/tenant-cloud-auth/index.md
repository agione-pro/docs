...
页面用于查看和维护租户与云平台之间的授权关系。使用列表底部的分页控件查看其他记录；翻页不会修改授权。
...
2. 在当前页搜索目标授权。
3. 当前页未找到目标记录时，先翻页检查，避免重复新增。
4. 点击 `新增租户云平台授权`。
5. 在弹窗中按页面要求选择 `选择云平台`。
6. 选择 `单个租户授权` 或 `授权所有租户`；如选择单个租户授权，继续填写 `选择租户`。
7. 点击最终 `确定` 前，再次核对云平台、授权对象和影响范围。
8. 如仅学习或验证页面，请点击 `取消` 或关闭弹窗，不提交真实授权配置。
...
| 分页可用 | 分页控件可打开其他结果页，且不会修改授权。 | 核对总数并重新加载列表。 |
