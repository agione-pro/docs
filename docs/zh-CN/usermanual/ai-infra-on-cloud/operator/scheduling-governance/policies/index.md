...
| 术语 | 说明 |
| --- | --- |
| 应用场景 | 成本优先、高可用生产或极速推理等调度目标。 |
| 主备路由 | 模型请求在主要和备用部署点之间的拓扑。 |
| 健康探测 | 用于判断部署点是否可继续接收请求的检测规则。 |
...
上图展示“策略管理”列表页面，展示多云调度策略卡片、应用场景类型、主备节点拓扑及运行状态，并提供“添加策略”入口。
...
1. 在策略管理页面点击 **"添加策略"**。
2. 填写策略名称，选择应用场景（如“成本优先”、“高可用生产”或“极速推理”）。
3. 选择需要绑定的目标模型及版本（如 `Qwen3-8b / v1.0`），下方将自动联动展示该模型在各云平台的可用路由节点资源列表。
4. 填写策略描述后点击 **"下一步"**。

![基础信息与模型关联](./images/basic-information-model-association-add.png)

上图展示“添加策略 - 步骤1：基础信息与模型关联”，录入策略名称、应用场景并关联目标模型，联动呈现各云平台可用节点。

#### 步骤二：定义主备路由拓扑

1. 在路由配置中，根据多云多地域的节点规格和成本，挑选主路由节点与备用路由节点。
2. 针对备路由节点配置优先级顺序（P1、P2 等），设定流量调度与自动降级策略。
3. 核对各节点的云厂商、规格、费用参考与推理框架版本，确认无误后点击 **"下一步"**。

![主备路由拓扑](./images/define-primary-backup-route-topology-add.png)

上图展示“添加策略 - 步骤2：定义主备路由拓扑”，为主路由与备用节点配置调度优先级与降级链路。

#### 步骤三：定义健康探测规则

1. 配置针对主备路由节点的健康探测参数，包括探测周期、超时时间及健康/不健康阈值。
2. 指定探测接口路径与期望响应状态码，确保异常节点能够及时被剔除并触发切流。
3. 确认探测规则无误后点击 **"提交"** 完成策略创建，返回列表核对新策略状态。

![健康探测规则](./images/define-health-probe-rules-add.png)

上图展示“添加策略 - 步骤3：定义健康探测规则”，配置健康检查探测周期、超时时间与故障切换判定阈值。
