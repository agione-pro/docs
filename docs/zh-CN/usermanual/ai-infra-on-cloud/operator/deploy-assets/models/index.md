...
上图展示“模型库”管理页面，展示已纳管的元模型卡片、所属系列、发布状态及关联的部署点信息，并提供“添加模型”入口。
...
上图展示模型详情抽屉，展示元模型基本信息、已配置的云平台部署点、算力方案、推理框架版本及健康探针状态。

### 添加模型

#### 步骤一：选择元模型

1. 点击模型库页面右上角 **"添加模型"** 进入配置向导。
2. 在元模型列表中通过作者、类型或关键词检索目标基础模型（如 `Qwen3-8b`），核对右侧呈现的上下文窗口、最大输入/输出约束和协议兼容性。
3. 选定后点击 **"下一步"**。

![选择元模型](./images/meta-model.png)

上图展示“添加模型 - 步骤1：元模型”，通过筛选并选定目标基础模型，右侧呈现其参数与能力约束。

#### 步骤二：配置部署点与算力

1. 在部署配置阶段点击 **"添加部署点"**。
2. 选择目标云平台（如阿里云）和已启用的地域资源池。

![添加部署点](./images/add-deployment-point.png)

上图展示在步骤2中点击“添加部署点”，选择云平台厂商及可用地域。

3. 选择云上预置模型或指定模型标识。

![选择云上模型](./images/assign-cloud-model.png)

上图展示指定部署点对应的云厂商模型标识或版本。

4. 挑选算力方案规格，并关联适用的推理框架版本。

![选择算力方案](./images/select-compute-plan.png)

上图展示为部署点选择算力方案规格与兼容的推理框架版本。

5. 按需添加输出配置，设置服务监听端口、超时时间等参数。

![添加输出配置](./images/add-output-configuration.png)

上图展示配置部署点的输出参数与通信协议规范。

#### 步骤三：确认配置并提交

1. 进入步骤3核对元模型、各云平台部署点、算力配置及框架版本的汇总信息。
2. 确认无误后点击 **"提交"**（或“上架版本”），完成模型录入。
3. 提交后回到模型库列表，确认新模型已处于可用状态。

![确认模型配置](./images/confirm-configuration.png)

上图展示“添加模型 - 步骤3：确认配置/上架版本”，对多云部署点和参数进行最终复核并提交。
