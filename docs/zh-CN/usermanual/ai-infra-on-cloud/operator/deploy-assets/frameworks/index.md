...
页面使用框架卡片展示列表。通过卡片或卡片中的 `详情` 进入详情页；详情页分别展示框架元数据和版本列表，版本操作与目标版本一起显示。
...
2. 使用云平台页签、`框架名称` 或 `框架类型` 定位框架卡片。
3. 从卡片打开框架详情。
4. 分别核对框架元数据和版本列表。
5. 打开编辑操作前，确认目标版本、运行镜像、启动命令和可用状态。
...
| 详情和版本正常加载 | 框架元数据和版本列表分别显示。 | 重新打开卡片，并检查页面数据权限。 |
