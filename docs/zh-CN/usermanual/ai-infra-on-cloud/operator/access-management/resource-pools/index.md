...
| 术语 | 说明 |
| --- | --- |
| 资源池 | 按云平台和地域组织的可用资源范围。 |
| 启用状态 | 资源池能否进入授权和调度流程的状态。 |
| 只看已启用 | 只显示当前已启用资源池的筛选项。 |
...
上图展示“接入资源池”管理页面，左侧选择接入的云平台厂商，右侧展示该平台下对应的地域资源池列表及其启用状态开关。
...
上图展示禁用或启用指定地域资源池时的二次确认提示框，需核对目标地域名称后再执行确认。
