...
| 术语 | 说明 |
| --- | --- |
| 部署状态 | 部署从创建到就绪或失败的当前阶段。 |
| 事件 | 记录调度、拉取、启动和异常信息的时间线。 |
| 发布区域 | 部署转为模型服务时选择的可见范围。 |
...
上图展示“我的部署”服务列表页面，按云平台筛选卡片，呈现部署实例名称、模型版本、节点规格、运行状态及每小时费用估算。
...
1. 在我的部署列表中定位运行中的目标实例卡片。
2. 点击卡片右下角更多操作图标（`···`），在展开菜单中选择 **"发布"**。

![发布入口](./images/publish.png)

上图展示在部署卡片的更多操作菜单中点击“发布”操作入口。

#### 步骤二：选择发布区域

1. 在“发布模型 - 选择发布区域”弹窗中，根据模型受众选择发布目标：
   - **私有区**：发布至私有模型库，仅当前组织/租户成员可见与调用。
   - **公共区**：发布至公共模型库，对全平台用户开放调用并可配置独立计价。
2. 点击对应的“发布到私有区”或“发布到公共区”进入详细配置页面。

![选择发布区域](./images/choose-where-to-publish.png)

上图展示选择发布区域弹窗，支持按需将模型发布到组织私有区或平台公共区。

#### 步骤三：完善模型发布信息并提交

1. 系统将自动带入元模型、模型源及上游预测服务请求 URL。
2. 录入或核对 API 密钥与模型源标识（模型源 ID）。
3. 依次完成计费配置与限流规则配置，确认无误后点击提交。
4. 提交后前往 `创作空间 > 我的模型` 列表，确认模型已成功上架并可正常调用。

![发布模型页面](./images/publish-model.png)

上图展示“发布模型”配置表单，自动填充部署服务请求 URL，并支持配置 API 密钥、计费模式与限流规则。
