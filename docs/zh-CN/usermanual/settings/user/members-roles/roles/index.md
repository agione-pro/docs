...
| --- | --- |
| 角色 | 一组页面和操作权限的集合。 |
| 角色标识 | 角色在列表中的识别字段，当前新增和编辑表单不提供此字段。 |
| 初始授权 | 创建角色时选择的首批页面、操作或数据权限。 |
| 授权范围 | 角色可以访问的页面以及可以执行的操作范围。 |
...
先使用角色名称筛选定位目标，再从结果列表进入编辑、授权或删除操作。角色创建与角色授权是两个独立任务，需要分别核对结果。
...
列表中的 `Default` 是示例角色记录；实际操作前应结合角色名称、角色标识和当前租户确认目标。
