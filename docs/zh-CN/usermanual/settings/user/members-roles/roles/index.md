...
角色页用于管理租户角色，支持按角色名称搜索，查看角色名称、角色标识、角色描述、创建时间，并通过添加角色表单创建新角色。
...
| 管理对象 | 租户角色、角色标识、角色描述和权限范围 |
| 典型途径 | 查询角色、查看角色权限、维护租户角色 |
...
1. 进入 `设置 > 成员与角色 > 角色`。
2. 按角色名称、状态或更新时间筛选。
3. 打开详情，核对菜单、操作权限和已关联成员。
4. 无结果时重置筛选；权限异常时检查成员是否关联多个角色。
...
角色创建在其他租户，角色被停用，或当前账号只有成员查看权限、没有角色管理权限。

**处理方式：**

确认租户范围和角色状态；检查当前账号是否具备角色查看权限；需要分配时由角色管理员处理。
#### 为什么角色新增或编辑按钮不可用？
