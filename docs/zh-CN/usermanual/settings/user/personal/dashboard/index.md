...
| 术语 | 含义 | 处理建议 |
| --- | --- | --- |
| 服务商账号 | 当前登录并查看设置概览的账号。 | 先确认是否处于正确租户。 |
| 可用额度 | 当前账号或租户可继续使用的额度。 | 额度不足时进入额度申请。 |
...
1. 进入 `设置 > 个人 > 概览`。
2. 选择目标项目或工作区，查看额度、用量、申请状态和快捷入口。
3. 对比总额度、已用和可用额度，确认统计对象一致。
4. 指标为空时检查项目选择和数据刷新时间；额度信息外发前必须脱敏。

### 下钻额度或用量异常

1. 点击额度、用量或申请状态的关联入口。
2. 保持相同项目上下文进入成员额度、额度申请或用量日志。
3. 核对异常是否由额度调整、项目切换或数据延迟造成。
4. 无法解释时记录脱敏项目和时间范围，不直接提交额度变更。
...
当前账号未加入项目，成员额度尚未分配，或租户未开放用户侧快捷入口。
...
当前成员没有对应功能权限，租户关闭了自助申请，或快捷入口依赖的项目、Key 尚未初始化。

**处理方式：**

按入口类型分别检查成员额度、项目权限和 Key 管理权限；缺少权限时联系租户管理员补充授权。
## 后续操作
