...
页面用于维护上游模型来源，包括来源名称和标识、区域、Base URL、请求头认证和连通性。列表中的名称、标识和 Base URL 支持复制。模型来源配置错了，后续模型发布和调用都会失败。

页面截图：

![模型来源列表](./images/model-source-list.png)

列表展示来源名称、来源标识、地域名称、Base URL、创建时间和操作。旧版 `返回旧版本` 入口已移除；行内管理以 `编辑` 和更多菜单为主，不再直接展示 `详情` 或 `删除` 按钮。
...
7. 点击 `确定` 前确认字段信息无误；如仅学习或验证页面，请点击 `取消` 关闭，不要把打开表单误认为已经保存。
