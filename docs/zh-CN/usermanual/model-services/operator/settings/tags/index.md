...
| 术语 | 说明 |
| --- | --- |
| 标签 | 用于标识模型能力、场景或行业的用户可见分类 |
| 标签类目 | 标签树的一级节点，用来组织一组相关子标签 |
| 子标签 | 挂在标签类目下的具体标签项，可用于模型筛选和展示 |
| 标签状态 | 页面在绑定和筛选控件中展示的标签状态 |
| 绑定关系 | 标签与模型、应用或展示入口之间的引用关系 |
...
上图展示“标签”配置管理页面，树形呈现标签类目与层级子标签，顶部支持状态与名称查询，右侧提供编辑、添加子标签、禁用和删除操作入口。
