...
| 术语 | 说明 |
| --- | --- |
| 应用审核记录 | 应用提交或变更进入审核流程后生成的记录。 |
| 绑定模型 | 应用配置中实际调用的模型或聚合模型。 |
| 可见范围 | 应用发布后可以看到该应用的客户或租户集合。 |
...
上图展示应用审核列表主界面，支持按审核状态（全部、待审核、审核通过、审核失败）切换查看，并提供应用名称/类目筛选、批量审核与单条审核入口。
...
![审核应用](./images/application-review-public.png)

上图展示应用审核详情与审批抽屉，包含审核信息、应用测试、应用详情页签以及底部的“拒绝”与“通过”操作按钮。
