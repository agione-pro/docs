...
页面用于处理模型上架、更新或下架审核，展示申请模型、供应方、元模型、来源凭据、模型可见范围、风险说明和审核意见。审核人应围绕授权、能力、合规和调用可用性给出明确结论。
...
2. 在模型审核列表中查看 `模型名称`、`模型可见范围`、`模型类型`、`客户名称`、`版本`、`免费配额`、`状态`、`提交时间`、`审核时间` 和 `操作`。
3. 通过 `全部`、`待审核`、`审核通过`、`审核失败` 状态页签筛选；再通过 `全部模型`、`公开`、`私有` 筛选模型可见范围，并按 `客户`、`模型名称` 或 `模型类型` 缩小范围。
...
完成第 4 步后，审核详情页用于核对模型基础信息、模型参数配置和审核操作。
...
| 模型可见范围 | 系统展示 | 枚举 | `全部模型` / `公开` / `私有` | 待审核模型的可见范围。 |
