...
| 术语 | 说明 |
| --- | --- |
| 模型审核记录 | 模型发布或变更进入审核流程后生成的记录。 |
| 模型来源 | 发布模型使用的上游来源和地域配置。 |
| 模型参数配置 | 模型输入输出模态、Token 限制、协议和能力设置。 |
| 可见范围 | 模型发布后可以看到该模型的客户或租户集合。 |
| 审核意见 | 通过、驳回或要求补充信息时记录的处理说明。 |
...
上图展示模型审核列表页面，支持按审核状态与模型可见范围（全部、公开、私有）切分查看，并提供客户、模型名称/类型多维筛选、单条审核及批量审核入口。
...
上图展示模型详情面板，展示基本信息与展开的“模型参数配置”，便于核对 Token 限制、模态及协议信息。
...
上图展示单条模型审核面板，核对模型参数与使用边界后，在底部选择“通过”或“拒绝”并填写审核意见。
...
上图展示“批量审核”配置弹窗，针对已勾选的多条模型审核记录统一选择审核结果并输入审批意见。
