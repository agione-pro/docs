...
上图展示“应用列表”发布管理页面，呈现已发布的应用卡片、分类标签、维护作者、计费状态及浏览收藏统计，并提供搜索、筛选器、置顶及标签编辑入口。
...
2. 在右上角搜索框中输入应用名称、作者、系列或来源关键字检索目标应用。
3. 如需更多筛选条件，点击 **"展开筛选器"** 按应用分类或状态过滤。
4. 核对结果卡片中的应用名称、分类标签、作者、计费状态与统计数据。

### 查看应用详情

1. 在目标应用卡片点击应用名称进入详情。
2. 查看应用描述、版本、关联模型、可见范围、发布状态和更新时间。
3. 对照列表核验版本与状态，确认配置符合运营规范。
4. 只读核验结束后返回列表。

### 置顶与编辑应用标签

1. 在目标应用卡片右侧，根据运营展示需求点击 **"置顶"**，系统将置顶项按序号优先排序展示在列表头部。
2. 点击 **"编辑标签"**，在弹窗中按需调整应用所属分类或业务标签。
3. 保存后刷新页面，核对应用在列表及客户端展示位置与标签变更。
