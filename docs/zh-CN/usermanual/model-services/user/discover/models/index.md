...
模型市场像可检索的模型目录。先找到符合输入输出能力的模型，再查看供应方、价格、上下文和可用状态，最后决定进入体验中心还是按快速开始信息接入。
...
第一次选型时，建议按“查询模型 → 查看模型详情 → 比较供应方实例 → 体验模型或查看快速开始 → 查看性能和概览”的顺序操作。不要只根据模型名称决定接入。

#### 小白先看

| 场景 | 先做 | 不要直接做 |
| --- | --- | --- |
| 不知道选哪个模型 | 先按输入、输出能力查询 | 直接按名称接入 |
| 同一模型有多个供应方 | 比较价格、状态和上下文 | 默认选择第一条记录 |
| 准备接入 API | 查看快速开始并使用占位符记录 | 把真实密钥写进文档 |
| 模型显示不可用 | 核对可见范围和供应方状态 | 连续提交调用 |
...
4. 核对结果中的模型名称、能力标签和可用状态；无结果时清除组合筛选后重新查询。

![查询模型](./images/manual-query-models.png)
...
![查看模型详情](./images/manual-model-details.png)
...
3. 需要体验或接入时，先在目标供应方卡片中确认模型 ID、协议和可用状态，再进入对应操作。

![模型供应方](./images/providers.png)
...
![体验模型入口](./images/manual-model-details.png)
...
![快速开始入口](./images/manual-model-details.png)
