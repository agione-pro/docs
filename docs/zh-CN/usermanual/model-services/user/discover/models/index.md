...
| --- | --- |
| 模型作者 | 模型的开发组织或品牌归属。 |
| 模型来源 | 提供可调用模型实例的上游服务来源。 |
| 上下文长度 | 模型单次请求可处理的输入与输出总量上限。 |
| 快速开始 | 展示调用地址、协议和认证方式的接入区域。 |
| Personal Key | 模型调用方在平台中发起模型调用时使用的个人凭据。 |
| API Key | API 认证密钥的通用名称。本页调用示例应使用 Personal Key；模型提供方发布模型时可能另需上游 API Key。 |
| 请求头 | 随请求发送的字段结构，常用于承载认证信息。 |
| Endpoint | 接收请求的实际目标地址，由 Base URL 和接口路径组成。 |
| Model ID | 调用时用于定位目标模型实例的标识。 |
...
1. 在模型列表中点击目标模型卡片，进入模型详情页。
2. 核对模型名称、作者、模型类型、输入输出能力、上下文长度和计费方式。
3. 确认详情页顶部展示模型摘要、能力标签、模态和协议信息。
4. 详情页下方提供【供应方】、【快速开始】、【性能】和【概览】四个页签，根据后续任务切换查看，不要把不同页签的信息混写。
...
核对供应方实例卡片中的价格、上下文、延迟与成功率指标，确认接入或体验的目标实例。
...
查看快速开始页签中的接入代码示例，核对 Base URL、请求路径及认证头格式。
...
核对选定时间范围与粒度下的平均响应延迟、首 Token 耗时及请求成功率趋势。
...
核对模型概览中的能力说明、支持模态与参数边界，确认模型特性符合业务要求。
