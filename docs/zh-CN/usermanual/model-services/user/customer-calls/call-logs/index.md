...
`客户调用 - 调用日志` 仅模型提供方可访问，用于查看客户侧模型调用记录，并排查客户反馈的失败、超时、限流或使用异常。普通用户只能在 `我的调用` 查看自己的记录。
...
3. 按页面筛选项选择月份、日期范围、模型、模型类型、调用状态或输入 Token 范围。
4. 组合使用 `最小输入 Token` 和 `最大输入 Token`，按输入 Token 范围缩小结果。
5. 点击 `搜索` 查看符合条件的客户调用日志。
6. 点击 `重置` 清空筛选条件；如需查看更多筛选项，点击 `展开`。
7. 点击目标日志的 `详情`，并隐藏请求、响应、Key、账号和费用等敏感内容。
