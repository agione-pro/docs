...
| 管理对象 | 客户模型调用记录、模型 ID、调用归属、客户账号、调用结果、用量和失败记录 |

#### 新手理解

客户调用日志像客户请求的排障明细单。先按时间、模型、模型 ID 和状态找到记录。客户名称表示客户账号或租户维度，调用归属表示该客户内部的成员、项目和 Key 范围。
...
| 模型 ID | 客户请求使用的平台模型标识。 |
| 客户名称 | 发起模型调用的客户账号或租户。 |
| 调用归属 | 该客户调用归属的成员、项目和 Key 范围。 |
| 调用结果 | 单次客户调用的成功或失败状态。 |
| 使用情况 | 单次调用的 Token、次数或其他消耗信息。 |
| 首 Token 时间 | 文本模型返回首个 Token 的耗时。 |
...
页面展示客户侧模型调用记录，支持按模型名称、模型 ID、模型类型、调用状态和时间范围查询。列表将模型 ID、调用归属和客户名称分开显示，避免混淆模型标识、客户内部归属和客户账号。
...
3. 点击 **"搜索"**，核对调用时间、模型名称、模型 ID、调用归属、客户名称、结果和使用情况。
4. 条件错误时点击 **"重置"**；共享结果前先脱敏客户与业务标识。

### 定位失败调用记录

1. 在调用状态中选择失败状态。
2. 点击 **"搜索"**，比较失败记录的客户名称、模型 ID、调用归属和时间分布。
3. 打开目标记录的 **"详情"**，先核对模型 ID 和调用归属，再只记录脱敏错误摘要；不要复制完整请求、响应或凭证。

![定位失败调用记录](./images/manual-failed-call-filter-public.png)

核对筛选失败状态后的日志记录，重点排查关联客户、模型标识与具体错误摘要。
...
| 模型 ID | 否 | 输入框 | `<模型 ID>` | 按平台模型标识精确筛选客户调用日志。 |
| 模型类型 | 否 | 选择项 | `对话模型` | 按模型能力类型筛选调用日志。 |
| 调用状态 | 否 | 选择项 | `全部` / `成功` / `失败` / `触发限流` | 按调用处理结果筛选日志。 |
| 最小输入 Token | 否 | 数字输入 | `0` | 设置查询的输入 Token 下限。 |
| 最大输入 Token | 否 | 数字输入 | `1000` | 设置查询的输入 Token 上限。 |
| 调用时间 | 系统生成 | 时间 | `2026-07-17 14:30` | 展示单次客户调用发生时间。 |
| 调用归属 | 系统生成 | 文本 | `成员 / 项目 / Key 范围` | 展示该调用关联的成员、项目和 Key 范围。 |
| 客户名称 | 系统生成 | 文本 | `Example Customer` | 展示发起调用的客户账号或租户，与调用归属分开。 |
...
| 日志列表可见 | 列表显示调用时间、模型名称、模型 ID、调用归属、客户名称、模型类型、调用结果和使用情况等列。 | 刷新页面，或调整月份、日期范围后重试。 |
...
| 字段信息一致 | 模型 ID、调用归属、客户名称、调用状态、使用情况、失败类型和错误信息与详情页一致。 | 重新打开详情或扩大时间范围交叉确认。 |
...
1. 点击 **"重置"** 并选择调用发生的日期。
2. 按 Model ID 和状态逐步筛选，再在返回记录中比较客户名称和调用归属。
