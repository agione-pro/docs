...
| 术语 | 说明 |
| --- | --- |
| 模型维度 | 按模型汇总客户调用量、成功、失败和限流。 |
| 客户维度 | 按客户汇总其调用模型和调用指标。 |
| 调用总量 | 当前统计范围内的客户调用总次数。 |
| TOP 排行 | 按调用量或其他指标排序的模型或客户。 |
...
![客户调用模型维度概览](./images/manual-model-analytics-public.png)

“概览”页面默认展示模型维度的客户调用概况，包含调用总量、成功率趋势及热门模型调用排行。

## 主要操作

### 查看模型维度调用概览

1. 进入 `模型及AI服务 > 客户调用 > 概览`，默认处于【模型维度】页签。
2. 设置统计月份与查询日期范围，可按目标模型进行筛选。
3. 核对调用总量、成功率、失败率、限流触发次数、调用趋势曲线及模型调用排行榜。
4. 如发现特定模型调用失败率或延迟异常，可点击行内详情进一步排查。

### 查看客户维度调用概览

1. 在页面顶部点击切换至【客户维度】页签。
2. 设置统计月份与查询日期范围，可按目标客户筛选。
3. 核对活跃客户总数、调用趋势图表、Token 消耗统计及客户调用排行榜。
4. 如发现特定客户调用异常激增或失败率上升，可点击行内详情进一步排障。

![查看客户维度调用概览](./images/manual-customer-analytics-public.png)

“客户维度”概览展示各客户的调用频次分布、Token 消耗总计及主要活跃客户排行榜。
