...
| 术语 | 说明 |
| --- | --- |
| 客户清单 | 按客户汇总模型使用数量和调用指标。 |
| 模型清单 | 按模型展示客户调用统计。 |
| 模型使用数量 | 客户在当前范围内调用过的模型数量。 |
| 查看详情 | 打开目标客户或模型的进一步统计信息。 |
...
![客户调用客户清单](./images/manual-customer-list-public.png)

“客户清单”页面展示当前周期内各客户调用模型的统计数据，包含使用模型数、成功/失败/限流次数及详情入口。

## 主要操作

### 查看客户调用清单

1. 进入 `模型及AI服务 > 客户调用 > 调用统计`，默认处于【客户清单】页签。
2. 可按客户名称或模型名称输入检索关键词，并设定统计月份或日期范围。
3. 点击 **"搜索"**，核对列表中各客户的模型使用数量、成功次数、失败次数及限流触发次数。
4. 点击目标客户操作列的 **"查看详情"** 可进一步查看该客户的细分调用记录；条件输入有误时点击 **"重置"**。

### 查看模型调用清单

1. 在“调用统计”页面顶部点击切换至【模型清单】页签。
2. 可按模型名称或模型类型筛选目标模型。
3. 点击 **"搜索"**，对比各模型的客户调用量、成功率及限流表现。
4. 点击行内“查看详情”可深入查看单模型在各客户间的调用分布情况。

![查看模型调用清单](./images/manual-model-list-public.png)

“模型清单”列表展示不同模型的被调用总次数、客户覆盖范围及调用成功率等关键指标。
