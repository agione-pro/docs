...
| 术语 | 说明 |
| --- | --- |
| On-Cloud | 部署在云平台并按云端资源运行的方式。 |
| On-Prem | 部署在自有物理机或虚拟化环境中的方式。 |
| 部署状态 | 部署任务当前所处的创建、运行、失败或停止状态。 |
| 调用信息 | 部署成功后用于接入模型的地址、协议和认证要求。 |
...
![我的部署列表](./images/manual-on-cloud-deployments-public.png)

“我的部署”页面默认展示 On-Cloud 云端模型部署列表，包含部署名称、关联模型、云平台、运行状态及管理操作。

## 主要操作

### 查看 On-Cloud 部署

1. 进入 `模型及AI服务 > 创作空间 > 我的部署`，默认位于【On-Cloud】页签。
2. 可按部署名称、模型或状态筛选云端部署实例。
3. 核对列表中部署名称、模型类型、云平台厂商、部署区域及当前运行状态。
4. 在操作列可执行启动、停止、查看详情或发起模型发布。

### 查看 On-Prem 部署

1. 在“我的部署”页面点击切换至【On-Prem】页签。
2. 列表中展示部署在本地私有算力池中的模型实例。
3. 核对目标部署的算力节点、GPU 规格卡数、容器健康状态及服务地址。
4. 针对异常或就绪的实例，可在操作列进行生命周期管理或发布操作。

![查看 On-Prem 部署](./images/manual-on-prem-deployments-public.png)

切换至 On-Prem 页签查看私有算力池中的本地模型部署实例、GPU 规格配额及服务就绪状态。

### 发布模型

1. 在目标运行中的部署卡片操作菜单中点击 **"发布"**。
2. 在弹出窗口中选择发布区域（私有区或公共区），确认目标可见范围。
3. 完善模型服务信息、API 密钥、计费方式及限流配置后提交。
4. 提交后可前往 `我的模型 > 我的发布` 查看已上架的模型服务状态。

![发布模型入口](./images/publish-public.png)

在部署卡片操作菜单中点击“发布”，启动模型发布流程。

![选择发布区域](./images/choose-where-to-publish-public.png)

选择模型发布至租户“私有区”或全平台“公共区”。

![发布模型页面](./images/publish-model-public.png)

核对自动填充的服务地址，完善访问密钥、计费方式及限流配置后提交。
