...
| 术语 | 说明 |
| --- | --- |
| 我的发布 | 当前模型提供方直接发布的模型清单。 |
| 我的聚合 | 由多个可用模型按策略组合形成的模型清单。 |
| 私有区 | 仅发布方所在租户成员可见和调用的范围。 |
| 公共区 | 可面向平台租户或指定租户提供模型的范围。 |
...
页面同时包含两个记录列表和两个多步骤创建流程。根据任务选择页签或入口：

| 页签 / 入口 | 用途 | 完成标志 |
| --- | --- | --- |
| 概览 | 开始发布模型或创建聚合模型。 | 打开目标发布区域选择弹窗。 |
| 我的发布 | 定位直接发布的模型，查看区域、状态和操作入口。 | 可看到目标记录及当前发布状态。 |
| 我的聚合 | 定位聚合模型，查看区域、状态和操作入口。 | 可看到目标聚合模型及当前状态。 |
| 发布模型 | 完成发布区域、基础信息、计费和限流阶段。 | 页面显示提交结果，并可到我的发布核对记录。 |
| 创建聚合模型 | 完成发布区域、候选模型、路由和计费阶段。 | 页面显示提交结果，并可到我的聚合核对记录。 |

页面截图：

![我的发布列表](./images/manual-my-published-public.png)

“我的发布”列表展示当前租户直接发布的模型实例、发布区域、运行状态及管理操作入口。

## 主要操作

### 查看直接发布模型

1. 进入 `模型及AI服务 > 创作空间 > 我的模型`，默认处于【我的发布】页签。
2. 可按模型名称、发布区域（私有区/公共区）或状态筛选定位已发布的模型记录。
3. 核对列表中各模型的名称、版本、来源、计费模式与上架状态。
4. 在操作列可执行编辑、停用、上下架或删除等模型管理操作。

### 查看聚合模型

1. 在“我的模型”页面顶部点击切换至【我的聚合】页签。
2. 列表中展示所有已配置的聚合模型，包含模型名称、路由模式、候选模型列表及当前状态。
3. 核对候选模型组的健康状态、分流权重及生效策略。
4. 切换页签仅改变当前查看的记录类型，如需调整可点击操作列进行编辑或删除。

![查看我的聚合](./images/manual-my-aggregate-public.png)

“我的聚合”列表展示已配置的聚合模型、候选模型组、路由分流策略与当前可用状态。

### 发布模型

按四个检查点完成流程：发布区域、基础信息、计费配置和限流配置。
...
| 发布区域 | `发布模型` 可打开区域选择弹窗，并可选择目标私有区或公共区。 | 检查发布权限及所需区域是否可用。 |
| 发布流程 | 基础信息、计费配置和限流配置阶段可见，并保留目标配置值。 | 返回上一阶段核对必填字段，必要时重新选择发布区域。 |
| 发布结果 | 页面显示提交结果，且我的发布中可看到目标记录和状态。 | 不要重复提交；检查页面提示、当前页签、筛选条件和审核状态。 |
| 聚合区域 | `创建聚合模型` 可打开区域选择弹窗，选择区域后显示兼容的候选模型。 | 确认兼容的已发布模型在所选区域中可见。 |
| 聚合流程 | 模型选择、行内控制、标签、名称预览、路由策略、Access Mode 和计费配置可见，并保留目标配置值。 | 检查候选模型是否满足同类元模型要求，并返回对应阶段核对。 |
| 聚合结果 | 页面显示提交结果，且我的聚合中可看到目标记录和状态。 | 不要重复提交；检查候选模型状态、当前页签、筛选条件和页面提示。 |
