...
`我的模型` 是模型提供方维护和发布模型的工作台。普通用户也可能看到概览，但受保护的模型创建入口不会打开创建流程。

| 项目 | 内容 |
| --- | --- |
| 适用角色 | 模型提供方、可查看受保护概览的普通用户 |
...
| 路由策略 | 聚合模型的路由策略：`价格优先`、`成功率优先`、`体验优先`、`随机` 或 `轮询`。 |
| 计费配置 | 设置 Token、免费、分层价格、缓存价格、免费额度等费用规则。 |
| 限流配置 | 设置请求频率、并发、配额等调用控制策略。 |

## 前提条件

1. 当前账号具备 `我的模型` 页面访问权限。模型提供方看到 `开始`，普通用户在受保护入口看到 `联系我们`。
...
页面包含 `概览`、`我的发布`、`我的聚合` 三个页签。`概览` 现在以轮播方式展示 `AGIOne 部署`、`自带密钥（BYOK）` 和 `聚合模型` 三张入口卡片。模型提供方可通过 `开始` 进入发布或聚合流程。普通用户可能看到相同的概览卡片，但受保护入口显示 `联系我们`；实际按钮取决于当前卡片和访问范围。License 和公共区身份校验也可能阻止继续操作。

::: tip 概览轮播
当前选中的卡片会自动切换。点击 `开始` 或 `联系我们` 前，请使用上一张/下一张按钮，并同时核对卡片标题和 `01 / 03` 至 `03 / 03` 计数。本手册不再使用固定的 `概览` 截图，避免中英文静态截图停在不同卡片上，造成当前选择状态不一致。
:::

`我的发布` 展示已发布模型记录、计费摘要、状态和可用操作。操作集合取决于记录状态和当前账号权限，应以行内实际显示为准。
...
4. 在 `模型选择` 中添加兼容的已发布模型。
5. 逐行核对 `启用`、`最低成功率`、`最高并发`、`上下文长度` 和 `价格权重`。
6. `价格权重` 是 1–100 的无量纲路由权重，不是计费金额。
7. 在 `基础信息` 中核对自定义或建议标签，以及模型名称预览。
8. 选择一种 `路由策略`：`价格优先`、`成功率优先`、`体验优先`、`随机` 或 `轮询`。
9. 根据页面的 `行为` 和 `适用场景` 说明选择策略。

![创建聚合模型 - 基础信息](./images/my-aggregate-basic-information-add.png)

10. 配置 Access Mode、发布方式和计费配置。

![创建聚合模型 - 计费配置](./images/my-aggregate-billing-configuration-add.png)

11. 点击最终 `创建`、`保存` 或 `提交` 前，再次核对参与模型、路由策略、价格和可见范围。模型保存前不要推断提交结果。
...
| 上下文长度 | 否 | 数字输入 | 按页面展示 | 展示或设置当前模型行的上下文长度边界。 |
| 价格权重 | 是 | 数字输入 | `1-100` | 设置无量纲路由权重，不代表计费金额。 |
| 自定义或建议标签 | 否 | 文本 / 标签 | `#低延迟` | 描述模型优势，并更新模型名称预览。 |
| 模型名称预览 | 系统生成 | 预览 | 按页面展示 | 预览聚合模型在列表和供应方卡片中的名称。 |
...
| 聚合字段正常显示 | 模型选择、行内控制、标签、名称预览、路由策略、Access Mode 和计费配置正常显示。 | 检查参与聚合模型是否满足同类元模型要求。 |
