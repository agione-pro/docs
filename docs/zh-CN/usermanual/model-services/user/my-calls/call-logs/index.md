...
| 管理对象 | 个人模型调用记录、模型 ID、调用归属、调用结果、用量、耗时和详情 |

#### 新手理解

调用日志像每次模型请求的明细单。先按模型、模型 ID、状态和时间找到记录，再通过调用归属区分该调用关联的成员、项目和 Key 范围。
...
| --- | --- |
| 调用时间 | 请求发起并被记录的时间。 |
| 模型 ID | 请求使用的平台模型标识，用于区分显示名称相近的模型。 |
| 调用归属 | 该调用归属的成员、项目和 Key 范围。 |
| 调用结果 | 单次调用的成功或失败状态。 |
| 首 Token 时间 | 文本模型返回首个 Token 的耗时。 |
| 调用详情 | 单次调用的模型、状态、用量、耗时和错误摘要。 |
...
页面按模型展示当前账号的调用记录，支持按模型名称、模型 ID、模型类型、调用状态和时间范围查询。列表和详情会将模型显示名称、模型 ID 与调用归属分开显示。
...
3. 点击 **"搜索"**，核对调用时间、模型名称、模型 ID、调用归属、结果和使用情况；条件错误时点击 **"重置"**。

### 查看调用详情

1. 在目标记录的操作列点击 **"详情"**。
2. 核对模型名称、模型 ID、调用归属、调用时间、结果、用量、耗时和错误摘要。
3. 排障记录只保留脱敏请求标识和错误摘要，不复制完整请求、响应或凭证。

![查看调用详情](./images/manual-call-log-details-public.png)

查看单次调用的详细信息面板，重点核对模型标识、请求耗时、Token 消耗及错误响应内容。
...
| 调用日志列表可见 | 列表显示调用时间、模型名称、模型 ID、调用归属、模型类型、调用结果和使用情况等列。 | 刷新页面，或调整月份、日期范围后重试。 |
...
| 字段信息一致 | 模型 ID、调用归属、调用状态、耗时、使用情况、失败类型和错误信息与详情页一致。 | 重新打开详情或扩大时间范围交叉确认。 |
