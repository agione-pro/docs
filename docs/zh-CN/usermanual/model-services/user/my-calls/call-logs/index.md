...
| 页面路由 | `/modelone/monitoring/calls/log` 跳转到 `/modelone/monitoring/calls/log/model` |
...
2. 入口路由会进入模型调用日志子路由 `/modelone/monitoring/calls/log/model`。
3. 在 `模型` 页签下查看调用时间、模型、模型类型、调用状态、使用情况、耗时时间、首 Token 时间、失败类型、错误信息和操作入口。
4. 组合使用 `最小输入 Token` 和 `最大输入 Token`，按输入 Token 范围缩小结果。
5. 点击 `搜索` 查看符合条件的调用日志。
6. 点击 `重置` 清空筛选条件；如需查看更多筛选项，点击 `展开`。
7. 点击目标日志的 `详情` 查看更多信息，并隐藏请求、响应、Key 和费用等敏感内容。
