...
| 术语 | 说明 |
| --- | --- |
| 我的收益概览 | 汇总当前时间范围内的收益指标和趋势。 |
| 收益详情 | 按时间和业务对象展示单条收益记录。 |
...
![模型收益概览](./images/manual-earnings-overview-public.png)

“我的收益概览”页面展示指定统计周期内的收益汇总指标、调用量趋势及模型收益分布图表。

## 主要操作

### 查看收益概览

1. 进入 `模型及AI服务 > 用量与收益 > 模型收益`，默认处于【我的收益概览】页签。
2. 选择统计账期与时间范围，核对总收益、调用总量及活跃模型数。
3. 查看收益趋势图表与模型分布占比，了解各模型产生的收益贡献。
4. 若图表无数据，可适当调整时间跨度或核对当前账号发布的模型是否有客户调用。

### 查询收益详情

1. 在页面顶部点击切换至【收益详情】页签。
2. 设置查询日期范围，可按用户名称、模型名称或模型类型进行精确过滤。
3. 点击 **"搜索"**，核对列表中的记录时间、调用用户、模型名称、计费规则及收益数值。
4. 可点击行内“计费规则”查看扣费明细；用于对账或分享时先对用户名称和标识进行脱敏。

![查询收益详情](./images/manual-earning-details-public.png)

“收益详情”列表展示每次模型调用产生的收益记录、对应用户、计费规则明细及消耗用量。
