...
| 管理对象 | 模型收益概览、收益详情、账期、日期、计量用量、计费规则、Credit、入账和待结转 |
| 典型途径 | 查看模型收益趋势、收益占比、租户消耗排行和收益明细 |
...
| 术语 | 说明 |
| --- | --- |
| 我的收益概览 | 展示租户数量、模型数量、收入 tokens、收入 Credit、收益趋势和收益占比。 |
...
| 计费规则 | 当前收益记录命中的计费单价、匹配条件和消耗用量。 |
| 计费规则详情 | 行内详情入口展开的规则明细，用于查看总计扣费、价格明细、匹配条件、消耗用量和费用汇总。 |
| Credit | 页面展示的收益或消耗单位。 |
...
页面包含 `我的收益概览` 和 `收益详情` 两个页签。`我的收益概览` 展示账期、日期、租户数量、模型数量、收入 tokens、收入 Credit、模型收益趋势、模型收益占比、租户活跃度、租户 Credit 消耗排行、模型调用次数趋势和模型调用次数分布，页面路由为 `/modelone/accounting/useage/overview/model`。`收益详情` 展示账期汇总、筛选条件、收益明细列表和行内计费规则详情入口，页面路由为 `/modelone/accounting/useage/detail/model`。
...
2. 打开 `我的收益概览` 页签。
3. 按页面筛选项选择 `账期` 和 `日期`。
4. 查看 `租户数量`、`模型数量`、`收入 tokens`、`收入 Credit` 等概览指标。
5. 查看 `模型收益趋势`、`模型收益占比`、`租户活跃度`、`租户 Credit 消耗排行（TOP10）`、`模型调用次数趋势` 和 `模型调用次数分布`。
6. 核对图表和排行数据时，不截取或对外发送未脱敏的用户、租户、金额或 Credit 明细。
...
5. 在收益明细列表中查看 `使用时间`、`模型`、`用户名称`、`延迟`、`计量用量`、`免费配额`、`计费模式`、`计费规则` 和 `credits` 等信息。
6. 点击行内 `View details · 3 details` 可查看 `计费规则详情`，包括总计扣费、价格明细、匹配条件、消耗用量、费用汇总、免费配额和总费用。
7. 如页面存在导出、结算或调整类入口，仅查看字段和状态，不执行结算、调账或导出敏感数据。
...
| 收益概览正常展示 | 租户数量、模型数量、收入 tokens、收入 Credit 和图表正常显示。 | 切换账期或日期后重试，确认当前周期是否有收益数据。 |
| 筛选项可用 | 账期、日期、用户名称、模型名称、模型类型等筛选项可输入或选择。 | 检查筛选条件格式，必要时点击 `重置` 后重新查询。 |
| 收益列表正常加载 | 收益详情列表展示使用时间、模型、用户名称、计量用量、免费配额、计费模式、计费规则和 credits。 | 确认账期内是否有收益记录，或放宽筛选条件。 |
