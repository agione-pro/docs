...
| 页面路由 | 概览：`/modelone/accounting/deduction/overview/model`<br>用量详情：`/modelone/accounting/deduction/detail/model` |
| 管理对象 | 用量概览、模型 ID、调用归属、消耗趋势、模型分布和用量详情 |

#### 新手理解

用量明细像模型业务的用量账本。概览用于先看总量和趋势，详情用于按时间、模型、模型 ID 和调用归属核对具体记录。

#### 术语表

| 术语 | 说明 |
| --- | --- |
| 消耗明细概览 | 汇总当前时间范围内的用量指标和趋势。 |
| 消耗明细详情 | 按时间和业务对象展示单条用量记录。 |
| 模型 ID | 用量记录关联的平台模型标识。 |
| 调用归属 | 用量记录归属的成员、项目和 Key 范围。 |
| 统计周期 | 决定概览与详情的时间范围和汇总口径。 |
| 模型分布 | 展示不同模型在总用量中的占比。 |
...
页面包含 消耗明细概览 和 消耗明细详情。概览展示汇总指标与趋势，详情列表新增模型 ID 和调用归属列。当前 Demo 尚未显示成员或项目归属筛选，仅使用页面可见的入账状态、模型名称和模型类型筛选。

页面截图：

![用量明细概览](./images/manual-usage-overview-public.png)

“消耗明细概览”页面展示当前账号在选定账期内的总消耗 Credit、调用总次数及模型消耗分布图表。

## 主要操作

### 查看用量概览

1. 进入 `模型及AI服务 > 用量与收益 > 用量明细`，默认处于【消耗明细概览】页签。
2. 选择统计账期并指定日期范围，查看总消耗 Credit、调用次数与平均每次调用消耗。
3. 查看消耗趋势图表与模型分布占比，掌握主要消耗的模型分布情况。
4. 若图表数据为空，可调整统计周期或核对当前账号在此期间是否有发起模型调用。

### 查询用量详情

1. 在页面顶部点击切换至【消耗明细详情】页签。
2. 设置查询日期范围，可按计入账单状态、模型名称或模型类型进行筛选。
3. 点击 **"搜索"**，核对列表中的使用时间、模型名称、模型 ID、调用归属、耗时、计量用量及扣减 Credit。
4. 如需共享明细或用于核算，共享前先对组织标识与业务敏感字段进行脱敏处理。

![查询用量详情](./images/manual-usage-details-public.png)

“消耗明细详情”列表展示每次调用的具体产生时间、模型标识、调用归属、耗时及消耗 Credit 记录。
...
| 模型 ID | 系统生成 | 文本 | `<模型 ID>` | 展示用量记录关联的平台模型标识。 |
| 调用归属 | 系统生成 | 文本 | `成员 / 项目 / Key 范围` | 展示用量记录归属的成员、项目和 Key 范围。 |
...
| 消耗明细可见 | 消耗明细列表显示使用时间、模型名称、模型 ID、调用归属、模型类型、账单口径、耗时、计量用量和 Credit。 | 确认账期内是否有消耗记录，或放宽筛选条件。 |
| 详情入口可打开 | 价格规则、详情或查看入口可正常展示相关信息。 | 检查记录是否完整，或刷新页面后重试。 |
| 字段与筛选一致 | 模型 ID、调用归属、消耗、费用、状态、时间等字段与筛选条件及相关调用日志一致。 | 对比调用日志和模型收益，确认统计延迟或计费规则差异。 |
