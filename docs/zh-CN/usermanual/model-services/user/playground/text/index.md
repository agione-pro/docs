...
| 术语 | 说明 |
| --- | --- |
| 模型实例 | 当前体验页面实际调用的模型与供应方组合。 |
| 提示词 | 描述任务、输入内容和输出要求的指令。 |
| 生成参数 | 控制生成结果长度、随机性、尺寸或其他行为的参数。 |
| Personal Key | 用于发起调用的个人授权密钥，文档示例统一写为 `<PERSONAL_KEY>`。 |
...
页面用于体验文本模型。当前页面可见模型选择器、复制模型 ID 图标、【多个模型对比】按钮、参数设置图标、个人密钥选择器和 Prompt 输入框；重点是选择模型与供应方，填写 Prompt，并按需调整 Temperature、Top-P 等生成参数，观察输入区、返回区、历史记录和错误提示。
...
4. 点击输入框左下角的参数设置图标（调音台图标），按需展开并调整 `Temperature`、`Top-P` 等生成参数。
...
| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 页面可进入 | `文本对话` 页面正常打开，左侧体验中心菜单、顶部模型选择区、【多个模型对比】按钮和参数设置图标控件可见。 | 确认账号权限、导航路径和页面加载状态。 |
| 模型选择器可加载 | 模型选择区可展开，能看到模型列表、供应方实例和状态信息。 | 刷新页面后重试，或确认目标模型是否对当前账号可见。 |
| 输入区和参数区可见 | Prompt 输入框、参数设置面板、Protocol、Temperature、Top-P、N、Stream 等字段可见。 | 检查页面是否加载完成，必要时切换模型后重新查看。 |
