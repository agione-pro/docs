...
| 术语 | 说明 |
| --- | --- |
| 指标名称 | 页面展示并被资源规格引用的指标名称。 |
| 指标类型 | CPU、内存、AI 加速卡指标或页面提供的其他指标类型。 |
| 资源 key | 调度或计量过程中使用的资源标识。 |
...
![新增规格指标](./images/manual-add-spec-metric.png)
...
![导入配置数据弹窗](./images/manual-import-spec-metric.png)

导入弹窗通过 **"读取文件"** 上传后端导出的 ZIP 配置包并做预检查；提交前核对指标类型、名称、单位、k8s-key 和 selector-key 是否与目标环境一致。

![导出配置包弹窗](./images/manual-export-spec-metric.png)

导出弹窗在 **"选择模块"** 中按资源池、模板、显存计算等分组勾选范围；导出前确认当前筛选条件，避免带出不必要的指标定义。
