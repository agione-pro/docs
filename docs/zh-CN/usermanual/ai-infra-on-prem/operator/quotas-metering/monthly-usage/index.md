...
月度计量用于按月份汇总租户资源消耗、Credits 折算和导出状态。运营管理员可以先查看月度汇总，再结合计量明细核对异常增长、跨周期资源或延迟入账。
...
2. 按账期月份或状态筛选月度汇总记录。
3. 查看列表中的账期、总用量、状态和创建时间，确认月度计量统计完整。

### 核对月度计量差异明细

1. 发现月度汇总与预期不一致时，保持相同月份与租户条件。
2. 前往 `计量明细` 或结合作业消费事件核对具体资源的计时和扣费记录。
3. 确认跨周期资源、延迟入账或统计口径是否一致；不通过直接修改底层数据库验证差异。
