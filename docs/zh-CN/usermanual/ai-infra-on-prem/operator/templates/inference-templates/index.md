...
| --- | --- |
| 模板 | 用户创建模型实例时选择的部署方案。 |
| 因子表单 | 用户创建实例时需要填写的参数集合。 |
| 动态表达式 | 根据用户输入、模型、精度或资源条件动态计算字段值或显示条件的表达式。 |
...
2. 点击右上角 **"创建推理模板"**。
3. 在 **模型** 下拉框中选择目标模型（如 DeepSeek-R1-Distill-Qwen-7B）。
4. 在 **模型版本** 下拉框中选择尚未创建推理模板的版本。
5. 在 **框架版本** 下拉框中选择适配的推理框架版本（如 vLLM · v0.13.0）。
6. 在 **模板名称** 中核对或自定义模板名称（默认自动填充模型名称）。
7. 核对模型、版本与框架匹配关系后，点击 **"保存"** 完成创建。

#### 操作截图

![创建推理模板](./images/manual-create-inference-template.png)

上图展示创建推理模板弹窗。需选择模型、模型版本、框架版本并确认模板名称，保存后将作为该模型版本的标准推理部署模板。
