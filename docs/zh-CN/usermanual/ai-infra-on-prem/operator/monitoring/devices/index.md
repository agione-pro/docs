...
2. 确认右上角地域与资源池，按所属集群、节点、设备类型、设备状态或时间范围进行筛选。
3. 查看设备列表与整体运行状态，核对设备 ID、设备类型、所属节点、所属集群、地域/可用区和设备状态。
...
1. 发现加速卡离线、温度过高、显存长期占满或出现驱动错误时，记录设备 ID 与对应节点。
2. 保持相同时间范围，切换到 `作业监控` 确认是否有大模型训练或高并发推理作业持续占用该卡。
3. 切换到 `节点统计`，检查宿主节点的负载、网络与驱动服务健康状态。
4. 确认硬件故障或掉卡时，通知运维工程师进行硬件隔离检修，避免影响后续作业调度。
