...
2. 按页面提供的 `实例类型`、`状态` 或关键字搜索目标实例，点击 **"搜索"**。
3. 在列表中查看实例名称、类型、运行状态、模型、规格、地域和创建时间。
4. 点击目标实例的 **"详情"**，核对模型版本、资源规格、实例状态、Endpoint 和关联事件。

### 排查与处理异常状态

1. 在实例详情中查看资源利用率、调用状态、事件和最近错误日志。
2. 保持相同时间范围，对照集群、节点和设备监控定位异常层级（如资源不足或显存溢出）。
3. 如需启动、停止、重启、扩缩容或删除实例，先确认对线上业务的影响并获得授权后再执行。
