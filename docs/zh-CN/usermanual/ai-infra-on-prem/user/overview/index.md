# 概览

## 功能概述

| 项目 | 内容 |
| --- | --- |
| 适用角色 | 模型提供方、模型调用方 |
| 导航路径 | AI基础设施 > On-Prem > 概览 |
| 页面路由 | `/powerone/overview` |
| 管理对象 | 部署模板、加速卡资源、快速入口、规格配额、资源用量趋势 |

#### 新手理解

概览页像异构卡纳管用户控制台的首页：上半部分告诉你当前能部署哪些模型、有哪些加速卡；中部提供在线 IDE 和运行实例的快捷入口；下方帮助你确认配额是否足够、资源有没有消费记录。

#### 术语表

| 术语 | 说明 |
| --- | --- |
| 规格 | 作业可申请的资源套餐，例如 CPU、内存、GPU 型号和卡数。 |
| 配额 | 租户可使用的资源上限，常见维度包括 GPU、CPU、内存和规格。 |
| 加速卡 | GPU、NPU 等 AI 计算设备，页面按厂商和型号展示。 |

#### 推荐操作顺序

1. 进入 `异构卡纳管 > 概览`。
2. 查看 `部署模板（Deployment Templates）`，确认是否有可部署模型。
3. 查看 `加速卡厂商（Accelerator Vendors）`，确认当前资源池支持的加速卡类型。
4. 通过 `在线 IDE（Online IDE）` 或 `运行实例（Runtime Instance）` 快捷入口进入创建流程。
5. 查看规格配额、消费趋势和资源用量，确认是否具备创建作业所需资源条件。

#### 小白先看

先确认当前任务是否涉及部署模板、加速卡资源、快速入口、规格配额、资源用量趋势，再按照推荐顺序操作。页面字段或状态与预期不符时，先回到前置对象页核对依赖，不要跳过结果校验直接进入下游流程。

## 前提条件

1. 当前账号能进入 `异构卡纳管` 子系统。
2. 运营管理员已为租户开放可见的地域、可用区、集群、规格和模板。
3. 如需创建在线 IDE、运行实例或模型实例，账号需要具备对应创建权限和可用配额。

## 页面说明

页面用于查看和处理部署模板、加速卡资源、快速入口、规格配额、资源用量趋势。

![概览](./images/overview-list.png)

页面上方展示可部署模板和加速卡厂商；中部提供在线 IDE、运行实例快捷入口；下方展示规格配额、消费趋势和资源用量。截图中可看到左侧用户端菜单、部署模板区域、加速卡区域和快速入口。

#### 页面区域

| 字段/区域 | 说明 |
| --- | --- |
| 部署模板 | 展示可部署模型和部署入口，未选择模型时部署按钮通常不可用。 |
| 加速卡厂商 | 按 NVIDIA、Huawei、AMD、Intel 等厂商展示加速卡型号和适配状态。 |
| 快速指南（Quick Guide） | 提供在线 IDE 和运行实例的创建入口。 |
| 规格配额（Specification Quota） | 展示规格级配额和已用量。 |
| 消费趋势（Consumption Trend） | 展示当前周期的消费趋势，若没有作业消费则显示为空。 |

## 主要操作

### 查看可部署资源

1. 进入异构卡纳管概览页，确认右上角地域与项目范围。
2. 在部署模板与加速卡厂商区域查看已适配的硬件型号、显存大小与可用状态。
3. 查看规格配额与资源消费趋势，确认是否有充足的算力余量。

### 排查配额或资源异常

1. 发现可用配额不足或无法部署目标规格时，记录规格型号与所属地域。
2. 前往 `配额与用量 > 配额用量` 核对当前租户或项目的具体分配额度与消耗记录。
3. 若配额充足但部署实例仍调度失败，结合监控与可用区状态排查集群物理卡是否已被全部占用。

## 参数速查表

| 字段名称 | 是否必填 | 字段类型 | 示例 | 说明 |
| --- | --- | --- | --- | --- |
| 地域 | 否 | 下拉选择 | `示例地域` | 概览统计地域。 |
| 资源类型 | 否 | 枚举 | `示例资源类型` | 概览资源类别。 |
| 配额 | 系统生成 | 数字 | `示例额度` | 当前账号可用配额。 |
| 已用量 | 系统生成 | 数字 | `示例已用量` | 当前已使用资源量。 |
| 更新时间 | 系统生成 | 日期时间 | `示例更新时间` | 概览数据更新时间。 |

## 踩坑提示

- 部署按钮不可用时，通常是尚未选中模型或加速卡。
- 配额充足不等于集群一定有空闲资源，创建失败时还需查看地域、可用区和规格。

- 不记录真实租户、地域、集群、规格、资源用量、消费数据或测试参数。
- 概览数据可能存在统计延迟，排障应进入实例详情、监控或配额页面复核。

## 结果校验

| 检查项 | 成功表现 | 异常时处理 |
| --- | --- | --- |
| 页面入口 | 可进入概览并看到筛选或统计区域 | 检查菜单权限、当前业务身份和所属租户范围 |
| 数据范围 | 列表或统计与所选时间、地域和对象一致 | 重置筛选并核对时间边界、时区和统计口径 |
| 数据更新 | 更新时间或最新记录符合预期周期 | 到来源页核对任务、计量或配额记录是否已生成 |
| 交叉核对 | 部署模板、加速卡资源、快速入口、规格配额、资源用量趋势可与详情、账单或监控记录对应 | 按对象标识和时间范围到对应详情页逐项比对 |

## 常见问题

#### 概览没有记录

**问题现象：**

页面正常打开，但列表或统计为空。

**可能原因：**

- 筛选范围过窄。
- 来源记录尚未生成。
- 当前角色不可见。

**处理方式：**

1. 重置筛选
2. 核对来源任务或计量周期
3. 检查当前业务身份和租户范围。

#### 概览范围不正确

**问题现象：**

显示的数据不属于预期时间、地域或对象。

**可能原因：**

- 时间边界不一致。
- 地域筛选未生效。
- 对象归属已变化。

**处理方式：**

1. 重新选择时间和地域
2. 核对对象标识
3. 到来源详情确认当前归属。

#### 概览更新延迟

**问题现象：**

来源操作已完成，但页面尚未出现最新记录。

**可能原因：**

- 统计任务尚未结束。
- 缓存未刷新。
- 来源状态仍在处理中。

**处理方式：**

1. 核对来源状态
2. 等待一个统计周期后刷新
3. 持续延迟时检查处理任务。

#### 详情或下载入口不可用

**问题现象：**

详情、展开或下载按钮不可点击。

**可能原因：**

- 当前记录不支持该操作。
- 角色权限不足。
- 文件尚未生成。

**处理方式：**

1. 选择可用记录
2. 检查角色权限
3. 确认对应统计或导出任务已经完成。

#### 汇总与明细不一致

**问题现象：**

汇总值与逐条明细相加结果不同。

**可能原因：**

- 统计周期不同。
- 存在舍入。
- 部分记录仍在处理。

**处理方式：**

1. 统一周期和时区
2. 按对象逐条核对
3. 等待未完成记录结算后复查。

## 注意事项

- 不要仅凭首页汇总判断故障原因，关键实例状态以对应详情页为准。
- 创建前先确认地域、规格和镜像来源，避免提交后因资源不可调度失败。
- 概览页只做汇总展示，不替代具体模块的创建、详情和排障页面。
- 模板、规格和加速卡由运营管理员配置，模型提供方或模型调用方无法在概览页直接修改。

## 后续操作

1. 进入 `模型部署 > 部署模板` 创建模型实例。
2. 进入 `开发资源 > 在线IDE` 创建交互式开发环境。
3. 进入 `配额&用量 > 资源配额` 查看资源上限。
