...
页面以表格展示资源规格、额度和操作入口。截图中可看到多个规格的额度显示为“不限”，操作列提供【消费明细】入口。
...
| --- | --- |
| 搜索区 | 按条件筛选资源规格。 |
| 资源规格 | 规格名称，例如 `128u.256g.8k-a100`。 |
| 额度 | 该规格的配额上限，未设置固定上限时显示为“不限”。 |
| 消费明细 | 打开该规格的消费明细弹窗，核对关联实例与时间。 |
| 分页区 | 用量记录较多时分页查看。 |
...
2. 找到目标规格。
3. 点击该行操作列的 **"消费明细"**。
4. 在明细弹窗中查看消费记录（包含使用量、消耗源、开始/结束消费时间）。
5. 根据实例名称、时间和资源类型继续排查。

下文配图展示消费明细入口打开后的页面状态，可用于查看规格用量来源。
