...
1. 进入 `配额&用量 > 资源配额`。
2. 查看显卡（AI card）、CPU、内存总量与已使用量。
3. 查看运行实例和在线IDE各自的资源占用项。
4. 点击 **"查看资源项占用项"** 查看详细占用列表。
...
页面按资源类型分为显卡、CPU、内存三个卡片，分别展示总量、未使用和已使用数量，并展示运行实例与在线IDE的资源占用情况。截图中各资源的总量均为“不限”，已使用量均为 0。

#### 页面区域

| 字段/区域 | 说明 |
| --- | --- |
| 显卡 | 展示 AI 加速卡名称、总量、未使用与已使用卡数。 |
| CPU | 展示 CPU 总体额度、已使用 vCPU 核心数与使用率仪表盘。 |
| 内存 | 展示内存总体额度、已使用 GiB 与使用率仪表盘。 |
| 运行实例 | 展示推理/训练运行实例占用的资源项，并提供查看入口。 |
| 在线IDE | 展示交互式在线开发环境占用的资源项，并提供查看入口。 |
| 查看资源项占用项 | 点击打开资源项占用详情弹窗，查看被占用的具体资源项列表。 |
...
1. 已确认目标资源类型，例如显卡、CPU 或内存。
2. 已确认需要查看运行实例还是在线 IDE 占用。

#### 操作步骤

1. 进入 `AI基础设施 > On-Prem > 配额&用量 > 资源配额`。
2. 找到【运行实例】或【在线IDE】区域。
3. 点击右侧的 **"查看资源项占用项"**。
4. 在【资源项占用详情】弹窗中核对占用资源项、状态及各硬件使用量。
5. 定位占用实例后进入对应实例列表处理。

下文配图展示资源项占用详情弹窗，用于查看实例维度的资源使用情况。
