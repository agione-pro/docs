...
| 结算单 | 某组织在某账期下的结算记录。 | [结算单列表](../../operator/finance-operations/settlement-list/) |
...
| 组织 | 条件必填 | 文本 | `脱敏组织` | 生成或核对结算单时用于限定对象。 |
...
3. 打开交易流水，按账期、交易类型、组织或流水号核对。
4. 如果账户金额和月结总览不一致，先确认账期、组织和交易类型是否一致。
...
- 财务账户、月结总览和结算单必须使用同一账期和组织范围。
- 调账只适合已定位且经过审批的差异，不适合作为常规修复入口。
- 生成结算单失败时不要重复点击，先查看生成检查和失败原因。
- `生成结算单`、补偿、补建和调账属于高风险动作，执行前必须确认账期、组织、金额和审批依据。
- 不记录真实组织、客户、金额、流水号、账户编号、结算单号、Token 或 Key。
...
| 结算单可追踪 | 能按账期和组织找到结算单，并打开详情。 | 归档或继续入账确认。 |
...
账期、组织、交易类型或账户类型选择不一致，也可能存在入账确认中、退款、调账或清算延迟。

**处理方式：**

先统一账期和组织；再打开财务账户交易流水核对；如果仍无法解释，进入巡检中心查看异常，并保留脱敏线索。
...
1. 账期闭环完成后，归档脱敏后的账期、组织、结算单状态、异常处理结论和复核人信息。
...
- 生成结算单、补建、补偿和调账前必须确认账期、组织和影响范围。
- 截图、工单、评论和交付记录中不要暴露真实组织、客户、金额、流水号、账户编号、结算单号或内部处理意见。
