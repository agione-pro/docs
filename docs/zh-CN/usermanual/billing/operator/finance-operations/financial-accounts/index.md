...
| 典型途径 | 核对账户余额、查看资金变化、定位交易流水、辅助结算和对账 |
...
6. 如仅学习或截图，只查看账户信息和流水，不执行调账、清算、确认入账等高风险操作。

下图展示平台清算账户区域，用于核对待清算资金和账户余额变化。

![平台清算账户](./images/platform-clearing-accounts-list.png)
...
6. 如仅学习或截图，只查看账户信息和流水，不执行调账、确认入账或导出敏感数据等高风险操作。
...
![交易流水详情](./images/platform-clearing-accounts-list.png)
...
| 账务差异无法解释 | [巡检中心](../reconciliation-center/) | 按账期、组织或交易类型排查差异 |
...
- 不记录真实账户 ID、客户名、组织名、账期金额、交易流水号、内部流水号、审批信息、账号、Token 或 Key。
