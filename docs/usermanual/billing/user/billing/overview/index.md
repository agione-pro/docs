...
`Overview` is used to view, filter, and maintain overview information. It helps user-side account, business admin, billing viewer work with overview records and related status from a consistent page entry.
...
1. The current account can access `Billing > Overview`.
