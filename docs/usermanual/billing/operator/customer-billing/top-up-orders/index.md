...
The filter area is above the table. To identify an externally created order, use the order scope, top-up source, external reference information, and channel text together.

| Field | Description |
| --- | --- |
| Top-up Order No. | Searches by order number. |
| Customer | Searches by customer name. |
...
| Posting Source | Top-up channel, such as `New User Registration`, `Stripe`, or `KooGallery`. |
| Top-up Amount | Amount in the original order currency. |
| Credited Amount | Credits actually posted to the account. |
| Status | Order status, such as `Succeeded` or `Canceled`. |
| Completion Time | Time when the order completed. |
| Creation Time | Time when the order was created. |
