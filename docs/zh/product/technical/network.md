...
- **高可用优先**：管理服务至少 3 节点，避免单点；共享存储独立网络通道。
...
> **⚠️ 重要约束**
>
> 1. 管理节点：不需要任何加速卡（GPU / TPU / NPU），但纳管的工作节点 **至少需 1 张加速卡**。
...
|---------------------------------------------------------------|:---:|:---:|:---:|:---:|:---:|----------------------------------------------|
| **AGIOne 核心管理节点**<br> (API Gateway + 核心服务<br>+ DB + 中间件 + 插件) | ≥ 3 | ≥ 16 核 | ≥ 16 GB | ≥ 100 GB | ≥ 400 GB | 公网入口：443/TCP<br>节点内网：千兆 LAN<br>出向：可访问第三方 API |
...
| 内部 | 负载均衡[可选]    | 管理节点 1/2/3 | TCP 80/443 / 业务端口 | L7 反向代理至各管理节点 |
...
- 入口启用 **推荐WAF / DDoS 防护**，按调用方下发独立 API Key（或 OAuth 2.0），并支持按租户的 **RPM/TPM 限流**。
...
管理节点资源由 **K8s 基础开销** 与 **AGIOne 服务开销** 两部分构成。所有规模均建议 **至少 3 节点** 部署以保证高可用。
...
|    端口     | 协议 | 服务                             | 源 → 目的 | 说明                                      |
|:---------:|:---:|--------------------------------|---|-----------------------------------------|
|  **443**  | TCP | AGIOne Web / API 入口            | 客户端 → 管控 / Ingress | OpenAI / Anthropic 兼容 API 入口；强制 TLS 1.3 |
| **6443**  | TCP | Kubernetes API Server/任务构建 API | 管控 ↔ 纳管节点             | K8s 控制面通信；管控平面下发调度 / 状态查询               |
| **32762** | TCP | AGIOne 服务流量转发                  | 管控 ↔ 纳管节点             | 流量转发                                    |
| **32761** | TCP | AGIOne 监控 / 指标采集               | 管控 ↔ 纳管节点             | GPU/NPU 利用率、显存、温度、Running Tasks 等       |
