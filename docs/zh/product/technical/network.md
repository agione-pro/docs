...
| --- | --- | --- | --- | --- |
| PoC All in One | 概念验证、功能演示、内部测试 | HTTP `18090` | 单节点本机访问为主 | 必须可访问互联网，用于拉取镜像与依赖 |
...
PoC 节点必须能够访问互联网，用于拉取镜像与依赖。PoC 模式不具备高可用和数据冗余能力，不建议用于生产。
...
| 管理节点出站 | 管理节点 | Internet / 镜像仓库 | HTTPS / 按需 | PoC 必须，生产建议保留 | 用于镜像拉取、升级、依赖下载 |
...
| 管理节点出站互联网 | PoC 必须；生产建议 >= 100 Mbps，用于镜像拉取与升级 |
